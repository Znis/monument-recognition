...
async def startup():
    ...
    global coords, bustoplist, monumentcolumnlist, restroomlist, hotellist, exchangelist, atmlist, nearbyserviceslisttemp
...
async def hitDb(tablename, ltlg, origimg):
    # query = tablename.insert().values(latlng=ltlg, origimg= bytearray([0]))
...
async def req_detail(category : Category):
    ...
    if category.category == "monument":
            ...
            for itemm in monumentcolumnlist:
                if(geopy.distance.geodesic(strtopoint(category.latilongi), strtopoint(itemm[8]) ).km * 1000 < 500):
    ...
    if category.category == "exchange":
            ...
            for itemm in exchangelist:
                 if(geopy.distance.geodesic(strtopoint(category.latilongi), strtopoint(itemm[1]) ).km * 1000 < 500):
